...
const TestCaseFormModal = ({
  ...
  useEffect(() => {
    if (open && testcase) {
      setLoading(true);
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api"}/assignment/${assignmentId}/testcase`,
        {
          credentials: "include",
        },
      )
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api"}/testcase/definitions/generate`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            prompt,
            project_type: assignmentProjectType,
    ...
    >
      <div className="flex items-center justify-between bg-primary03 px-12 py-3 text-white">
        <div className="flex items-center gap-2">
          ...
          <Tooltip
            title={
              "This is a Robot Framework script generated by the system based on the assignment name and detail. " +
              "If the testcase is incorrect, you can prompt the AI to regenerate it, or manually edit it directly in the code editor."
            }
            arrow
            slotProps={{
              tooltip: {
